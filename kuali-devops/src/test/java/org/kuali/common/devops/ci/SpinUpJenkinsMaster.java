...
public class SpinUpJenkinsMaster {
	...
	protected String findLatestSlaveAMI(EC2Service service, Tag stack) {
		...
		reverse(slaveImages);
		checkState(slaveImages.size() > 0, "expected at least one slave image but there were zero");
		return slaveImages.get(0).getImageId();
	}
	...
	protected static Map<String, JenkinsContext> getJenkinsContexts(Tags.Name name) {
		...
		JenkinsContext prod = JenkinsContext.builder().withRegion(region).withDnsPrefix("ci").withStack(Tags.Stack.PROD).withName(name).build();
