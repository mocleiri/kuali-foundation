 ...
 * @version $Id: KualiGenerator.java,v 1.1.2.1 2008-03-11 19:38:19 lprzybyl Exp $
 */
public class KualiGenerator {
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(KualiGenerator.class);
     ...
     */
    private static final String DEFAULT_TEXEN_PROPERTIES = "org/apache/texen/defaults/texen.properties";
    ...
    /**
     * Keep track of the file writers used for outputting to files. If we come across a file writer more then once then the
     * additional output will be appended to the file instead of overwritting the contents.
     */
     ...
     */
    private static KualiGenerator instance = new KualiGenerator();
     ...
     */
    public static KualiGenerator getInstance() {
        instance = new KualiGenerator();
        return instance;
    }
    ...
    /**
     * Create a new generator object with properties loaded from a file. If the file does not exist or any other exception occurs
     * during the reading operation the default properties are used.
     * 
     * @param propFile properties used to help populate the control context.
     */
    public KualiGenerator(String propFile) {
        try {
     ...
     */
    public KualiGenerator(Properties props) {
    ...
    /**
     * Parse an input and write the output to an output file. If the output file parameter is null or an empty string the result is
    ...
    /**
     * Parse an input and write the output to an output file. If the output file parameter is null or an empty string the result is
     * returned as a string object. Otherwise an empty string is returned. You can add objects to the context with the objs
     * Hashtable.
     * 
     * @param inputTemplate input template
     * @param outputFile output file
     ...
     * @throws Exception
     */
    public String parse(String inputTemplate, String inputEncoding, String outputFile, String outputEncoding, String objectID,
        ...
        if (objectID != null && object != null) {
            ...
            // System.out.println("Adding objectID " + objectID + " to the context");
        ...
        if (outputFile == null || outputFile.equals("")) {
            ...
            // System.out.println("Using a writer");
            Writer writer;
            if (writers.get(outputFile) == null) {
                /*
                 * We have never seen this file before so create a new file writer for it.
                 */
                writer = getWriter(getOutputPath() + File.separator + outputFile, outputEncoding != null ? outputEncoding
                        : this.outputEncoding);
        ...
        controlContext.remove(objectID);
     ...
     * @throws Exception
     */
    public void parse(String controlTemplate, Context controlContext, Writer writer) throws Exception {
        ...
        template.merge(controlContext, writer);

    }

    public String parse(String controlTemplate, Context controlContext) throws Exception {
        StringWriter sw = new StringWriter();
        parse(controlTemplate, controlContext, sw);
        return sw.toString();
    }

    /**
     * Create a new context and fill it with the elements of the objs Hashtable. Default objects and objects that comes from the
     * properties of this Generator object is also added.
     * 
     ...
     * 
     * @param context control context to fill with objects that are specified in the default.properties file
     */
    ...
    /**
     * Properly shut down the generator, right now this is simply flushing and closing the file writers that we have been holding on
     * to.
     */
