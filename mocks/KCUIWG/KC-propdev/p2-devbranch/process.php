    ...
    case "removeComplianceEntry":
     ...
     case "addAttachmentsEntry":
           ...
           foreach($_REQUEST as $index=>$field){
             ...
             $_SESSION['attachments'][$_REQUEST['section']][] = $list;
             $id = max(array_keys($_SESSION['attachments'][$_REQUEST['section']]));
             $entry = $_SESSION['attachments'][$_REQUEST['section']][$id];
             include "inc/attachments." . $_REQUEST['section'] . ".entry.php";

        break;
    case "editAttachmentsEntry":
            $id = $_REQUEST['id'];
              $entry = $_SESSION['attachments'][$_REQUEST['section']][$id];
              //print_r($entry);
              $actionLabel = "Update entry";
              $action = "updateAttachmentsEntry";
            include('themes/kc/inc/header.php');
             include('inc/attachments.' . $_REQUEST['section'] . '.form.php');
             include('themes/kc/inc/footer.php');

    break;
    case "previewAttachmentsEntry":
              $id = $_REQUEST['id'];
              $entry = $_SESSION['attachments'][$_REQUEST['section']][$id];
             //  print_r($entry);
              include('inc/attachments.' . $_REQUEST['section'] . '.preview.php');
        break;
    case "updateAttachmentsEntry":
                foreach($_REQUEST as $index=>$field){
                   $list[$index] = trim($field);
               }

               $list["uploadFile"] = $_SESSION['attachments'][$_REQUEST['section']][$_REQUEST['id']]["uploadFile"];
               $list['uploadTime'] = $_SESSION['attachments'][$_REQUEST['section']][$_REQUEST['id']]['uploadTime'];

               $_SESSION['attachments'][$_REQUEST['section']][$_REQUEST['id']] = $list;
                $id = $_REQUEST['id'];
                $entry = $_SESSION['attachments'][$_REQUEST['section']][$id];
                include "inc/attachments." . $_REQUEST['section'] . ".entry.php";
        break;
        case "removeAttachmentsEntry":

            if(isset($_REQUEST["id"])){

                $id = $_REQUEST["id"];
                if(isset($_SESSION['attachments'][$_REQUEST['section']][$id])) unset($_SESSION['attachments'][$_REQUEST['section']][$id]);
            }
...
//            $list['uploadTime'] = date("n/j/Y g:i A");
//
//            $_SESSION['attachments']['notes'][] = $list;
//            $id = max(array_keys($_SESSION['attachments']['notes']));
//            $entry = $_SESSION['attachments']['notes'][$id];
