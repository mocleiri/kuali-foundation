 ...
 * <br>
 * Type 1: DDL statements for creating tables, primary keys, indexes, and unique constraints. Does not contain DDL for enforcing relationships between tables.<br>
 * Type 2: DDL statements for creating and enforcing relationships between tables<br>
 * <br>
 * This allows data to be imported into multiple tables concurrently. Running the first type of SQL file will create the empty tables without any foreign key constraints. Data can
 * then be loaded concurrently into the tables (using optimized high speed tools if desired) without needing to worry about the order in which the tables are loaded. After data has
 * been loaded, the second type of SQL file can be run to add the relationships between the tables. As long as the data set is consistent and correct, all the relationships will
 * get created correctly.<br>
 * <br>
...
public class SchemaSqlMojo extends SqlMojoBase {
	 ...
	 * 
	 * @parameter expression="${reportFile}" default-value= "../../../reports/report.${project.artifactId}.sql.generation"
	 */
	@SuppressWarnings("unused")
	 ...
	 * 
	 * @parameter expression="${contextPropertiesPath}" default-value="${project.build.directory}/reports/context.sql.properties"
	 */
	@SuppressWarnings("unused")
