...
public class DumpTaskTest {
	...
	public void test() {
		try {
			...
			p.setProperty("project.build.directory", p.getProperty("project.basedir") + "/target/impex");
	...
	protected DumpContext getDumpContext(Properties properties) {
		File schemaXmlFile = new File(properties.getProperty("project.build.directory") + "/schema.xml");
		...
		context.setSchemaXmlFile(schemaXmlFile);
		context.setSchemaName(db);
	...
	protected Task getSchemaDumpTask(DumpContext context, Project project) {
		...
		task.setIncludePatterns(Arrays.asList(context.getInclude()));
		return task;
	}
