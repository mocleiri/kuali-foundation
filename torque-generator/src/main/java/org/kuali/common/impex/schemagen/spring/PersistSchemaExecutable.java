...
import org.kuali.common.impex.schemagen.SchemaPersistenceException;
...
@Configuration
@Import(PersistSchemaConfig.class)
public class PersistSchemaExecutable implements Executable {

    @Autowired
    PersistSchemaConfig persistSchemaConfig;

    @Autowired
    PersistSchemaService persistService;
    ...
    public void execute() {
        ...
        for (String location : schemaLocations.keySet()) {
            ...
            try {
                persistService.writeSchema(schemaLocations.get(location), writer);
            } catch (SchemaPersistenceException e) {
