...
public class Composer {
	...
	@Inject
	public Composer(ConstructorComposer constructorComposer, BuilderComposer builderComposer, WithMethodPredicate withMethodPredicate) {
	...
	public void compose(ComposerRequest request, DialogContent dialogRequest, FlattenedICompilationUnit flattenedICompilationUnit, JavaClassFile javaClassFile)
		...
		if (request.isCreateConstructorWithBuilder()) {
			String constructorWithBuilderBuilder = constructorComposer.composeFromScratch(request, type.getElementName());
			type.createMethod(constructorWithBuilderBuilder, flattenedICompilationUnit.getBuilderType(), false, null);
		}
		if (flattenedICompilationUnit.getConstructorWithBuilder() != null
				&& (!request.getMissingAssignmentsInConstructor().isEmpty() || !request.getExtraFieldsInBuilder().isEmpty())) {
			String sourceLines = constructorComposer.composeFromExisting(request, javaClassFile.getMainType().getConstructorWithBuilder());
		...
		for (Field each : new TreeSet<Field>(request.getMissingWithMethodsInBuilder())) {
			...
			builder.createMethod(composeWithMethod, flattenedICompilationUnit.getBuildMethod(), false, null);
		}
		if (request.isCreateBuildMethodInBuilder()) {
			String composeBuilderMethod = builderComposer.composeBuilderMethod(javaClassFile.getMainType(), request.isCreateValidateMethodInBuilder());
		...
		if (request.isCreateValidateMethodInBuilder() || !request.getMissingFieldValidationsInBuild().isEmpty()) {
