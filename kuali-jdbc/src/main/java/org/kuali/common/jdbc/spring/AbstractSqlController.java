...
@Import({ JdbcCommonConfig.class, JdbcDataSourceConfig.class,
		SqlDbaBeforeConfig.class, SqlDbaAfterConfig.class })
@Deprecated
public abstract class AbstractSqlController {
	...
	protected Executable getSqlExecutable() {
		...
		List<SqlExecutionContext> contexts = SqlConfigUtils
				.getSqlExecutionContexts(env);

		for (SqlExecutionContext context : contexts) {
			SqlConfigContext scc = new SqlConfigContext(env, context,
