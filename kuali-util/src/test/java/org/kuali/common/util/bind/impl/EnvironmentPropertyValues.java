...
public final class EnvironmentPropertyValues<T> extends MutablePropertyValues {
	...
	public static <T> EnvironmentPropertyValues<T> create(Class<T> type, BindingPrefix bind, Environment env) {
		return builder(type, bind, env).build();
	}

	public static <T> Builder<T> builder(Class<T> type, BindingPrefix bind, Environment env) {
	...
	public static class Builder<T> implements org.kuali.common.util.build.Builder<EnvironmentPropertyValues<T>> {
		...
		public Builder(Class<T> type, BindingPrefix bind, Environment env) {
			this.type = type;
			this.env = env;
			this.bind = bind;
		}
		...
		public EnvironmentPropertyValues<T> build() {
			this.values = BindUtils.getMap(type, bind, env);
