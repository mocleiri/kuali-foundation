...
public abstract class CommandLineUtils {
	...
	public static int executeCommandLine(Commandline cl, InputStream systemIn, StreamConsumer systemOut, StreamConsumer systemErr, int timeoutInSeconds)
			throws CommandLineException {
		final CommandLineCallable future = executeCommandLineAsCallable(cl, systemIn, systemOut, systemErr, timeoutInSeconds);
		return future.call();
	}
	 ...
	 * @param timeoutInSeconds
	 *            Positive integer to specify timeout, zero and negative integers for no timeout.
	 * @return A CommandLineCallable that provides the process return value, see {@link Process#exitValue()}. "call" must be called on this to be sure the forked process has
	 *         terminated, no guarantees is made about any internal state before after the completion of the call statements
	 * @throws CommandLineException
	 *             or CommandLineTimeOutException if time out occurs
	 * @noinspection ThrowableResultOfMethodCallIgnored
	 */
	 ...
	 * i.e. <code>getSystemEnvVars().get("path")</code> and <code>getSystemEnvVars().get("PATH")</code> will in general return different values. However, on platforms with
	 ...
	 * @see System#getenv() System.getenv() API, new in JDK 5.0, to get the same result <b>since 2.0.2 System#getenv() will be used if available in the current running jvm.</b>
	 */
	...
	/**
	 * Return the shell environment variables. If <code>caseSensitive == true</code>, then envar keys will all be upper-case.
	 * 
	 * @param caseSensitive
	 ...
	 * @deprecated Use {@link StringUtils#quoteAndEscape(String, char, char[], char[], char, boolean)}, {@link StringUtils#quoteAndEscape(String, char, char[], char, boolean)}, or
