...
/**
 * <p>
 * Maven Ant Mojo. Allows Maven to invoke a target inside an Ant build file. The build file can be located on the file
 * system, the ant-maven-plugin classpath, or any resource URL that Spring 3.0 can understand.
 * </p>
 *
 * <p>
...
public class AntMojo extends AbstractMojo {
     ...
     * For example, a prefix of "mvn" means that the attachartifact task is referenced by "&lt;mvn:attachartifact&gt;"
     * The default value of an empty string means that no prefix is used for the tasks.
     *
     ...
     *
     * If this value is 'false', the Maven build will proceed even if the ant build fails. If it is 'true', then the
    ...
    /**
     * The build file to use. This supports Spring 3.0 resource URL expressions eg "classpath:build.xml" or
     * "http://myurl/build.xml". The ant-maven-plugin classpath is what is searched when using the "classpath:" notation
     *
    ...
    /**
     * The target inside the build file to invoke. If not provided, the default target from the specified build file
     * will be executed
     *
    ...
    /**
     * If true, pass Maven object references to Ant (MavenProject, MavenProjectHelper, ArtifactRepository - for the
     * local repo)
     *
    ...
    /**
     * If they give us "http://myurl/mybuild.xml" this gets set to "target/ant/mybuild.xml" by the handleAntFile()
     * method
     */
    ...
    protected AntTaskPojo getAntTaskPojo() {
        ...
        pojo.setDir(project.getBasedir().getAbsolutePath());
        return pojo;
    }
