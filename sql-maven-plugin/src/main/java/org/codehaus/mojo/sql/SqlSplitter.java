...
public class SqlSplitter {
	 ...
	 * that we do <em>not</em> fully parse the SQL, so if we get a malformed
	 ...
	 * @return position after the end character if the given line contains the
	 *         end of a SQL script, {@value SqlSplitter#NO_END} if it doesn't
	 *         contain an end char.
	 */
			...
			if (line.startsWith(delimiter, pos)) {
				if (isAlphaDelimiter) {
					...
					if ((pos == 0 || !isAlpha(line.charAt(pos - 1)))
							&& (line.length() == pos + delimiter.length() || !isAlpha(line
									.charAt(pos + delimiter.length())))) {
	 ...
	 * @param c
	 *            the char to check
	 * @return <code>true</code> if the given character is either a lower or an
	 *         upperchase alphanumerical character
	 */
